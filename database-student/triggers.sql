...
-- Valida stock antes de insertar item
...
-- Descuenta o devuelve stock al mover items
...
-- Calcula precio * cantidad automáticamente
...
-- Recalcula el total del pedido completo
...
BEGIN
    ...
    SELECT COALESCE(shipping_cost, 0) INTO v_shipping
    FROM orders 
    WHERE id = v_order_id;
...
-- Mantiene estadísticas del cliente al día
CREATE OR REPLACE FUNCTION update_customer_stats_on_order()
RETURNS TRIGGER AS $$
BEGIN
    -- Si cambia el estado o se crea un pedido, actualizar stats
...
BEGIN
    IF NEW.order_number IS NULL OR NEW.order_number = '' THEN
        ...
        -- Contar pedidos de este año para la secuencia
        SELECT COUNT(*) + 1 INTO v_sequence 
        FROM orders 
        WHERE EXTRACT(YEAR FROM created_at) = EXTRACT(YEAR FROM CURRENT_DATE);
        
        -- Formatear: ORD-2025-0042
