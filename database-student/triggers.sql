...
BEGIN
    IF TG_OP = 'DELETE' THEN
        ...
        VALUES ('products', OLD.id, 'DELETE', row_to_json(OLD)::jsonb);
        RETURN OLD;
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO audit_log (table_name, record_id, action, old_values, new_values)
        VALUES ('products', NEW.id, 'UPDATE', row_to_json(OLD)::jsonb, row_to_json(NEW)::jsonb);
        RETURN NEW;
    ELSIF TG_OP = 'INSERT' THEN
        INSERT INTO audit_log (table_name, record_id, action, new_values)
        VALUES ('products', NEW.id, 'INSERT', row_to_json(NEW)::jsonb);
        RETURN NEW;
    END IF;
...
-- =====================================================
-- TRIGGER 9: set_order_number (OPCIONAL - ver nota)
-- =====================================================
-- NOTA: Este trigger requiere que la tabla orders tenga el campo order_number
-- Como nuestro schema no lo tiene, este trigger es opcional
...
BEGIN
    ...
    IF NEW.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Número de orden generado: %', v_order_number;
    END IF;
...
-- Comentado porque el schema no tiene order_number
