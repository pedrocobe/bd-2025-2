...
export const ReportsQueries = {
  ...
  topCustomers: `
    ...
    GROUP BY c.id, customer_name, c.email, c.city, c.country
  ...
  inventoryAnalysis: `
    SELECT
      ...
      COALESCE(SUM(p.stock_quantity * p.cost), 0) AS inventory_cost,
      COALESCE(SUM(p.stock_quantity * p.price), 0) AS inventory_value,
      COALESCE(SUM(p.stock_quantity * (p.price - p.cost)), 0) AS potential_profit,
      COUNT(CASE WHEN p.stock_quantity < p.min_stock_level THEN 1 END) AS low_stock_items
    FROM categories c
    LEFT JOIN products p ON c.id = p.category_id AND p.is_active = true -- Condición movida aquí
    GROUP BY c.id, c.name -- Agregado c.id por buena práctica
