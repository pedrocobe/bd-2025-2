...
export const CategoriesQueries = {
   ...
   */
  findAllWithParent: `
      -- Variante: devolver parent como objeto JSON (manteniendo parent_name en estructura)
      SELECT c.id AS id,
        c.name AS name,
        c.description,
        c.parent_id,
        parent.name AS parent_name,
        c.is_active,
        json_build_object('id', parent.id, 'name', parent.name) AS parent_info
      FROM categories c
   ...
   */
  findHierarchy: `
    -- Variante: usar CTE recursivo para poder obtener niveles múltiples
    ...
    SELECT id, name AS category_name, parent_id AS parent_name, path AS full_path
    FROM cat_tree
