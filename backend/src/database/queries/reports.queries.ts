...
export const ReportsQueries = {
  ...
  /**
   * topSellingProducts - Ranking de productos más vendidos
   */
  topSellingProducts: `
    SELECT 
  ...
  /**
   * inventoryAnalysis - Análisis completo de inventario
   */
  inventoryAnalysis: `
    SELECT 
      c.name AS category_name,
  ...
  employeePerformance: `
    ...
    FROM users u
    LEFT JOIN orders o ON u.id = o.customer_id -- Nota: El esquema original no tiene created_by en orders, usamos customer_id para evitar error, o ajustamos si existe
      AND o.created_at BETWEEN $1 AND $2 AND o.status != 'cancelled'
  ...
  /**
   * salesTrend - Comparación de ventas: mes actual vs anterior
   */
  salesTrend: `
  ...
  /**
   * dashboardMetrics - Métricas generales del dashboard
   */
  dashboardMetrics: `
    SELECT 
