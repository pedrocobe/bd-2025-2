...
export const CustomersQueries = {
   ...
   */
  findAll: `
    -- Variante bd-4: paginación con LIMIT/OFFSET hint (no parametrizado aquí)
   ...
   */
  updateStatistics: `
    -- Variante bd-4: usar JOIN con subquery agregada
    UPDATE customers c
    ...
    WHERE c.id = s.customer_id AND c.id = $1
    RETURNING c.id, c.total_spent, c.order_count
  `,
