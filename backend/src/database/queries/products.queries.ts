...
export const ProductsQueries = {
  ...
  findById: `
    SELECT 
      p.id, p.name, p.description, p.sku, p.category_id, p.price, p.cost,
      p.stock_quantity, p.min_stock_level, p.is_active, p.created_at,
      p.updated_at, p.created_by, c.name AS category_name
    FROM products p
    LEFT JOIN categories c ON p.category_id = c.id
    WHERE p.id = $1
  `,
   ...
   */
  findBySku: `
    SELECT id, name, description, sku, category_id, price, cost,
           stock_quantity, min_stock_level, is_active, created_at,
  ...
  create: `
    ...
    RETURNING id, name, description, sku, category_id, price, cost, stock_quantity, 
              min_stock_level, is_active, created_at, updated_at, created_by
  `,
