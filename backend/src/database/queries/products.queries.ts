...
export const ProductsQueries = {
   ...
   */
  findById: `
    -- Variante bd-4: usar json_build_object para category info y evitar *
    SELECT p.id, p.name, p.description, p.sku, p.category_id, p.price, p.cost, p.stock_quantity,
           p.min_stock_level, p.is_active, p.created_at, p.updated_at, p.created_by,
           json_build_object('id', c.id, 'name', c.name) AS category_info
    FROM products p
    LEFT JOIN categories c ON p.category_id = c.id
    WHERE p.id = $1
  `,
   ...
   */
  findLowStock: `
      -- Variante bd-4: calcular deficit con GREATEST y filtrar
      SELECT p.id, p.name, p.sku, p.stock_quantity, p.min_stock_level,
        GREATEST(p.min_stock_level - p.stock_quantity, 0) AS deficit,
        c.name AS category_name
      FROM products p
      LEFT JOIN categories c ON p.category_id = c.id
   ...
   */
  getInventoryValue: `
      -- Variante bd-4: usar GROUP BY c.id, c.name para mayor robustez
